...
USE [DBAtools] -- Specify the database in which the objects will be created.
...
BEGIN
  ...
  -- BELOW MODIFIED BY HSO

  SET @JobName09 = 'HSO - Cleanup SQL Jobs'
  ...
  SET @JobName10 = 'HSO - Output File Cleanup'
  ...
  SET @JobName11 = 'HSO - CommandLog Cleanup'
  SET @JobCommand11 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "DELETE FROM [DBAtools].[dbo].[CommandLog] WHERE StartTime < DATEADD(dd,-30,GETDATE())" -b'
...
IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName09)
  BEGIN
    EXECUTE msdb.dbo.sp_add_job @job_name = @JobName09, @description = @JobDescription, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled=0
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName09, @step_name = @JobName09, @on_success_action=3, @subsystem = 'CMDEXEC', @command = @JobCommand09, @output_file_name = @OutputFile09
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName09, @step_name = @JobName10, @on_success_action=3, @subsystem = 'CMDEXEC', @command = @JobCommand10, @output_file_name = @OutputFile10
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName09, @step_name = @JobName11, @on_success_action=1, @subsystem = 'CMDEXEC', @command = @JobCommand11, @output_file_name = @OutputFile11
	EXEC	msdb.dbo.sp_add_jobschedule @job_name=@JobName09, @name=N'Once in a week', @freq_interval=1, @active_start_time=20000, @freq_type=8, @freq_recurrence_factor=1 
